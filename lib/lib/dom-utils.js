...
export function updateDomProperties(dom, prevProps, nextProps) {
  ...
  const isAttribute = name => !isEvent(name) && name !== "children"; // 移除旧事件和属性
  ...
  Object.keys(prevProps).filter(isAttribute).forEach(name => dom[name] = null); // 添加新事件和属性

  Object.keys(nextProps).filter(isEvent).forEach(name => {
    const eventType = name.toLowerCase().substring(2);
    dom.addEventListener(eventType, nextProps[name]);
  });
  Object.keys(nextProps).filter(isAttribute).forEach(name => dom[name] = nextProps[name]);
}
/**
 * 创建dom元素
 * @param {*} fiber
